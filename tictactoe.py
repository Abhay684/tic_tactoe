...
class Game(Tk):
    ...
    def gameover_screen(self, outcome):
        ...
        self.canvas.create_text(int(WINDOW_SIZE/2), int(WINDOW_SIZE/2),text=wintext, fill='black',font=('Calibri',int(-WINDOW_SIZE/6),'bold'))
        self.canvas.create_text( int(WINDOW_SIZE/2),int(WINDOW_SIZE/1.65),text='[Click to play again]',fill='purple', font=('Arial',int(-WINDOW_SIZE/25)) )
